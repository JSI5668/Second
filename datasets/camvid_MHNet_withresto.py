...
class Camvid_MHNet_withresto(data.Dataset):
    ...
    **Parameters:**
    ...
    def __init__(self, root, split='train', mode='fine', target_type='semantic', transform=None):  #추가 1: root_2
        ...
        self.targets_dir_for_restoration = os.path.join(self.root + '/', 'gtFine_For_Restoration' + '/', split) ## 추가 4
        self.transform = transform

        self.split = split
        self.images = []
        self.targets = []
        self.targets_for_resto = [] ##추가 6
        # self.images_2 = [] ## 추가 5
        # self.targets_2 = [] ##추가 6
        ...
        for city in os.listdir(self.images_dir):
            ...
            target_dir_for_restoration = os.path.join(self.targets_dir_for_restoration + '/', city)

            for file_name in os.listdir(img_dir):
                self.images.append(os.path.join(img_dir + '/', file_name))
                self.targets.append(os.path.join(target_dir + '/', file_name))
                self.targets_for_resto.append(os.path.join(target_dir_for_restoration + '/', file_name))
    ...
    def __getitem__(self, index):
        ...
        target_for_restoration = Image.open(self.targets_for_resto[index]).convert('RGB')
        image_trash = Image.open(self.images[index]).convert('RGB')


        if self.transform:
            image, target = self.transform(image, target)
            target_for_resto, image_trash = self.transform(target_for_restoration, image_trash) #추가 9

        target = self.encode_target(target)


        return image, target, target_for_resto
