...
from network.unet.unet_parts import *
import torchvision.transforms.functional as TF

from torch.utils import data
from datasets import   Kitti_sample, Camvid_proposed,  Camvid_sample, Camvid_Edge
...
PATH_1 = 'E:/Second_paper/Checkpoint/Camvid_edgeNetwork/sobel_model_firstfold/model.pt'
model_Edge = torch.load(PATH_1)
model_Edge.cuda()
...
def get_argparser():
    ...
    parser.add_argument("--data_root", type=str, default='E:/dataset/psf_15_3',
    ...
    parser.add_argument("--dataset", type=str, default='camvid_sample',
                        choices=['camvid_sample', 'Edge', 'Edge_laplacian', 'camvid', 'camvid_sample', 'camvid_focus', 'kitti_sample', 'camvid_proposed', 'camvid_proposed_each_0'], help='Name of dataset')
    parser.add_argument("--dataset_edge", type=str, default='Edge',
                        choices=['camvid_sample', 'Edge', 'Edge_laplacian', 'camvid', 'camvid_sample', 'camvid_focus',
                                 'kitti_sample', 'camvid_proposed', 'camvid_proposed_each_0'], help='Name of dataset')
    ...
    # parser.add_argument("--ckpt", default='E:/Second_paper/Checkpoint/Camvid_edgeNetwork/sobel_model_firstfold/model.pt', type=str,
    #                     help="restore from checkpoint")
    parser.add_argument("--ckpt",default='E:/Second_paper/Checkpoint/Camvid/main_10_unetmy2_1,0.5,0.25,/best_deeplabv3plus_resnet50_camvid_sample_os16.pth', type=str,
                        help="restore from checkpoint")
...
def validate(opts, model, loader, device, metrics, ret_samples_ids=None):
    ...
    with torch.no_grad():
        for i, (images, labels) in tqdm(enumerate(loader)):
            ...
            # edge_input = torch.max(outputs,1,keepdim=True)[0]
            # edge_pre = model_Edge(edge_input).max(1)[1].cpu().numpy()
            # edge_pre = Camvid_Edge.decode_target(edge_pre).astype('uint8')
            # edge_pre = Image.fromarray(edge_pre[0])
            # edge_pre.show()
            #
            # edge_pre_2 = model_Edge(edge_input)
            # edge_pre_2 = Conv1x1(in_channels=2, out_channels=1)(edge_pre_2.cpu())
            # edge_pre_2 = nn.Sigmoid()(edge_pre_2)
            # edge_pre_2 = (edge_pre_2>0.5).float()
            # edge_pre_2 = Camvid_Edge.decode_target(edge_pre_2).astype('uint8')
            # edge_pre_2 = Image.fromarray(edge_pre_2[0])
            # edge_pre_2.show()
...
def main():
    ...
    # model_edge = model_Edge(n_channels=1, n_classes=2, bilinear=True)
    ...
    if opts.ckpt is not None and opts.test_only:
        ...
        # print(model)
        # torch.save(model, 'E:/Second_paper/Checkpoint/Camvid/EdgeNet_laplacian_model/model.pt')
...
######### last checkpoint 받아서 이어서 학습 ##############
        ...
        while True: #cur_itrs < opts.total_itrs:
            ...
            for (images, labels) in train_loader:
                ...
                params = transforms.RandomCrop.get_params(images, output_size=(opts.crop_size, opts.crop_size))

                images = TF.crop(images, *params)
                labels = TF.crop(labels, *params)

                ab = np.random.uniform(0.2, 0.9)

                images = TF.adjust_brightness(images, ab)
                images = TF.adjust_contrast(images, ab)
                images = TF.adjust_saturation(images, ab)
                ...
                labels = transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])(labels)
...
###################################################################################
                ...
                if (cur_itrs) % opts.val_interval == 0:
                    ...
                    save_ckpt('E:/Second_paper/Checkpoint/Camvid/Blurred/main_10_unetmy2_withoutperceptual/latest_%s_%s_os%d.pth' %
                              (opts.model, opts.dataset, opts.output_stride))
                    print("validation...")
                    model.eval()
                    val_score, ret_samples = validate(
                    ...
                    if val_score['Mean IoU'] > best_score:  # save best model
                        ...
                        #           (opts.model, opts.dataset,opts.output_stride))
                        save_ckpt('E:/Second_paper/Checkpoint/Camvid/Blurred/main_10_unetmy2_withoutperceptual/best_%s_%s_os%d.pth' %
                                  (opts.model, opts.dataset, opts.output_stride))
                    ...
                    save_ckpt('E:/Second_paper/Checkpoint/Camvid/Blurred/main_10_unetmy2_withoutperceptual/_%s_%s_%s_os%d.pth' %
                ...
                if cur_itrs >= opts.total_itrs:
                    ...
                    df_train_miou.to_csv('E:/Second_paper/Checkpoint/Camvid/Blurred/main_10_unetmy2_withoutperceptual/train_miou_2.csv', index=False)
                    df_train_loss.to_csv('E:/Second_paper/Checkpoint/Camvid/Blurred/main_10_unetmy2_withoutperceptual/train_loss_2.csv', index=False)
