...
from network.unet import MHNet_my, MHNet_my_2, UNet_my_2, MHNet
...
def get_argparser():
    ...
    parser.add_argument("--data_root", type=str, default='E:/Second_paper/Data/CamVid/Blurred/Firstfold/For_Segmentation_With_Resto_typedeeplab',   ##Deeplab으로 바꿀때 model backbone, optimizer 설정
                        help="path to Dataset")  ##crop size 바꿔주기
    # parser.add_argument("--data_root", type=str,
    #                     default='D:/Dataset/Camvid/camvid_original_240',
    #                      help="path to Dataset")   ##crop size 바꿔주기

    parser.add_argument("--dataset", type=str, default='camvid_MHNet_withresto',
                        choices=['voc', 'cityscapes, camvid', 'camvid_sample', 'camvid_focus', 'kitti_sample', 'camvid_proposed', 'camvid_MHNet_withresto'], help='Name of dataset')
    ...
    parser.add_argument("--total_itrs", type=int, default=351000,
    ...
    parser.add_argument("--ckpt", default='E:/Second_paper/Checkpoint/Camvid/Blur_firstfold/main_11_with_res_MHNet_my_2/best_deeplabv3plus_resnet50_camvid_MHNet_withresto_os16.pth', type=str,
    ...
    parser.add_argument("--val_interval", type=int, default=350,
...
def get_dataset(opts):
    ...
    if opts.dataset == 'camvid_MHNet_withresto':
        ...
        train_dst = Camvid_MHNet_withresto(root=opts.data_root, split='train', transform=train_transform)

        val_dst = Camvid_MHNet_withresto(root=opts.data_root, split='val', transform=val_transform)

        test_dst = Camvid_MHNet_withresto(root=opts.data_root, split='test', transform=test_transform)
...
def validate_With_resto(opts, model1, model2, loader, device, metrics, ret_samples_ids=None):
    ...
    with torch.no_grad():
        for blurred_image, segment_target, restore_target in loader:

            images_resto = blurred_image.to(device, dtype=torch.float32)
            labels_resto = restore_target.to(device, dtype=torch.float32)
            labels_seg = segment_target.to(device, dtype=torch.long)

            outputs_pre_resto = model1(images_resto)[0]
###########################################################################################################
            # outputs_pre_resto = outputs_pre_resto.squeeze(0).permute(1,2,0).cpu().numpy()
            # labels_seg = labels_seg.squeeze().cpu().numpy()
            #
            # outputs_pre_resto = outputs_pre_resto * 255
            # outputs_pre_resto = outputs_pre_resto.astype(np.uint8)
            #
            # outputs_pre_resto = Image.fromarray(outputs_pre_resto)
            # labels_seg = Image.fromarray(labels_seg, 'L')
            #
            #
            # outputs_pre_resto, labels_seg = val_transform(outputs_pre_resto, labels_seg)
            # outputs_pre_resto = outputs_pre_resto.unsqueeze(0).to(device, dtype=torch.float32)
            # labels_seg = labels_seg.unsqueeze(0).to(device, dtype=torch.long)
###########################################################################################################
            outputs_pre_resto.to(device, dtype=torch.float32)
            # outputs_pre_resto = normalize_val(outputs_pre_resto)
...
####################resto#################################
    # train_dataset_resto = get_training_data('E:/Second_paper/Data/CamVid/Blurred/Firstfold/For_Segmentation_With_Resto/train', {'patch_size':opts.crop_size})
    # train_loader_resto = DataLoader(dataset=train_dataset_resto, batch_size=opts.batch_size, shuffle=True, num_workers=2,
    #                           drop_last=False, pin_memory=True)
    # val_dataset_resto = get_validation_data('E:/Second_paper/Data/CamVid/Blurred/Firstfold/For_Segmentation_With_Resto/test', {'patch_size':opts.crop_size})
    # val_loader_resto = DataLoader(dataset=val_dataset_resto, batch_size=opts.val_batch_size, shuffle=False, num_workers=2)
...
####################resto#################################
    ...
    optimizer_pre = optim.Adam(model_pre.parameters(), lr=2e-4, betas=(0.9, 0.999),eps=1e-8)
    warmup_epochs = 3
    scheduler_cosine = optim.lr_scheduler.CosineAnnealingLR(optimizer_pre, 1000 - warmup_epochs,eta_min=1e-6)
    scheduler_pre = GradualWarmupScheduler(optimizer_pre, multiplier=1, total_epoch=warmup_epochs, after_scheduler=scheduler_cosine)
...
######### last checkpoint 받아서 이어서 학습 ##############
        ...
        while True: #cur_itrs < opts.total_itrs:
            ...
            for blurred, semantic_target, resto_target in train_loader:
                cur_itrs += 1
                ...
                outputs_pre = model_pre(input_)[0]
                # tf(outputs_pre[0]).show()

                loss_resto = criterion_char(outputs_pre, target) + 0.05 * criterion_edge(outputs_pre, target)
                loss_resto.backward()
                optimizer_pre.step()
...
#######################################################################################################
                # outputs_pre = outputs_pre.squeeze(0).permute(1, 2, 0).cpu().numpy()
                # target_seg = target_seg.squeeze().cpu().numpy()
                #
                # outputs_pre = outputs_pre * 255
                # # target_seg = target_seg * 255
                #
                # outputs_pre = outputs_pre.astype(np.uint8)
                # # target_seg = target_seg.astype(np.uint8)
                #
                # outputs_pre = Image.fromarray(outputs_pre)
                # target_seg = Image.fromarray(target_seg, 'L')
                # # outputs_pre.show()
                #
                # outputs_pre, target_seg = train_transform(outputs_pre, target_seg)
                # # tf(outputs_pre[0]).show()
                # outputs_pre = outputs_pre.unsqueeze(0).to(device, dtype=torch.float32)
                # target_seg = target_seg.unsqueeze(0).to(device, dtype=torch.long)
#######################################################################################################
                outputs_pre = outputs_pre.to(device, dtype=torch.float32)
                semantic_target = semantic_target.to(device, dtype=torch.long)
                # target_seg = target_seg.squeeze(0).to(device, dtype=torch.long)
                # normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406],
                #                    std=[0.229, 0.224, 0.225])
                # outputs_pre = normalize(outputs_pre)

                optimizer.zero_grad()
                outputs = model(outputs_pre)
                ...
                if (cur_itrs) % opts.val_interval == 0:
                    ...
                    save_ckpt('E:/Second_paper/Checkpoint/Camvid/Blur_firstfold/main_11_with_res_MHNet_my_2/latest_%s_%s_os%d.pth' %
                              (opts.model, opts.dataset, opts.output_stride))
                    print("validation...")
                    model.eval()
                    val_score, ret_samples = validate_With_resto(
                    ...
                    if val_score['Mean IoU'] > best_score:  # save best model
                        ...
                        #           (opts.model, opts.dataset,opts.output_stride))
                        save_ckpt('E:/Second_paper/Checkpoint/Camvid/Blur_firstfold/main_11_with_res_MHNet_my_2/best_%s_%s_os%d.pth' %
                                  (opts.model, opts.dataset, opts.output_stride))
                    ...
                    save_ckpt('E:/Second_paper/Checkpoint/Camvid/Blur_firstfold/main_11_with_res_MHNet_my_2/_%s_%s_%s_os%d.pth' %
