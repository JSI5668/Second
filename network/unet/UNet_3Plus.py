...
class UNet_3Plus_my(nn.Module):

    def __init__(self, in_channels=3, n_classes=1, feature_scale=4, is_deconv=True, is_batchnorm=True):
        super(UNet_3Plus_my, self).__init__()
    ...
    def forward(self, inputs):
        ...
        x_down1 = F.interpolate(inputs, scale_factor=0.5, mode='bilinear', align_corners=False)
        x_down2 = F.interpolate(inputs, scale_factor=0.25, mode='bilinear', align_corners=False)
        x_down3 = F.interpolate(inputs, scale_factor=0.125, mode='bilinear', align_corners=False)

        h2 = self.maxpool1(h1)
        h2 = self.conv2(h2)  # h2->160*160*128

        x_down1 = self.SCM1(x_down1)
        x2 = torch.cat([h2, x_down1], dim=1)
        x2 = self.Conv1_2(x2)
        x2_CA = self.CA_1(x2)
        x2_ = x2 * x2_CA
        x2 = x2 + x2_
