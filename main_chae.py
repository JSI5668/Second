...
def get_argparser():
    ...
    parser.add_argument("--data_root", type=str, default='D:/Dataset/Camvid/camvid_sample_2/blur_new/camvid_firstfold_15_3',
...
def main():
    ...
    model_dec_freeze = decoder_my_2(n_classes=12, bilinear=False)
    checkpoint = torch.load(opts.ckpt_dec, map_location=torch.device('cpu'))
    model_dec_freeze.load_state_dict(checkpoint["model_state"])
    model_dec_freeze.cuda()
    model_dec_freeze.eval()
    for param in model_dec_freeze.parameters():
        param.requires_grad_(False)

    model_enc_training = encoder_my_2(n_channels=3, bilinear=False)
    checkpoint = torch.load(opts.ckpt_enc, map_location=torch.device('cuda'))
    model_enc_training.load_state_dict(checkpoint["model_state"])
    model_enc_training.cuda()
    ...
    optimizer = optim.RMSprop(model_enc_training.parameters(), lr=opts.lr, weight_decay=1e-8, momentum=0.9)
    ...
    def save_ckpt_encoder(path):
        ...
        torch.save({
            "cur_itrs": cur_itrs,
            # "model_state": model.module.state_dict(),  ##Data.parraell 이 있으면 module 이 생긴다.
            "model_state": model_enc_training.state_dict(),
            "optimizer_state": optimizer.state_dict(),
            "scheduler_state": scheduler.state_dict(),
            "best_score": best_score,
        }, path)
...
######### last checkpoint 받아서 이어서 학습 ##############
        ...
        while True: #cur_itrs < opts.total_itrs:
            ...
            for (images, labels) in train_loader:
                ...
                training_enc_output_1, training_enc_output_2, training_enc_output_3, training_enc_output_4, training_enc_output_5 = model_enc_training(images)

                enc_feature_map_loss = F.mse_loss(freeze_enc_output_5, training_enc_output_5)
                outputs = model_dec_freeze(training_enc_output_1, training_enc_output_2, training_enc_output_3, training_enc_output_4, training_enc_output_5)
                loss = criterion(outputs, labels)
...
###################################################################################
                ...
                ## train miou
                if (cur_itrs) % opts.val_interval == 0:
                    train_val_score, ret_samples = validate_chae(
                        opts=opts, encoder=model_enc_training, decoder=model_dec_freeze, loader=train_loader, device=device, metrics=metrics,
                ...
                if (cur_itrs) % opts.val_interval == 0:
                    ...
                    save_ckpt_encoder('E:/Second_paper/Checkpoint/Camvid/Blur_firstfold/asd/latest_%s_%s_os%d.pth' %
                              (opts.model, opts.dataset, opts.output_stride))
                    print("validation...")
                    model.eval()
                    val_score, ret_samples = validate_chae(
                        opts=opts, encoder=model_enc_training, decoder=model_dec_freeze, loader=test_loader, device=device, metrics=metrics, ret_samples_ids=vis_sample_id)
                    print(metrics.to_str(val_score))
                    ...
                    if val_score['Mean IoU'] > best_score:  # save best model
                        ...
                        #           (opts.model, opts.dataset,opts.output_stride))
                        save_ckpt_encoder('E:/Second_paper/Checkpoint/Camvid/Blur_firstfold/asd/best_%s_%s_os%d.pth' %
                                  (opts.model, opts.dataset, opts.output_stride))
                    ...
                    save_ckpt('E:/Second_paper/Checkpoint/Camvid/Blur_firstfold/asd/_%s_%s_%s_os%d.pth' %
                ...
                if cur_itrs >= opts.total_itrs:
                    ...
                    df_train_miou.to_csv('E:/Second_paper/Checkpoint/Camvid/Blur_firstfold/asd/train_miou_2.csv', index=False)
                    df_train_loss.to_csv('E:/Second_paper/Checkpoint/Camvid/Blur_firstfold/asd/train_loss_2.csv', index=False)
