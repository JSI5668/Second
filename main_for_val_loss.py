...
from datasets import VOCSegmentation, Cityscapes, Camvid, Camvid_60,  Camvid_sample, Camvid_focus, Kitti_sample, Camvid_deblur85_v2_blending
...
def get_argparser():
    ...
    parser.add_argument("--data_root", type=str, default='D:/Dataset/KITTI/segmentation/firstfold/original_2/asdfsf',
                        help="path to Dataset")  ##crop size 바꿔주기
    # parser.add_argument("--data_root", type=str,
    #                     default='D:/Dataset/Camvid/camvid_original_240',
    #                      help="path to Dataset")   ##crop size 바꿔주기
    parser.add_argument("--dataset", type=str, default='kitti_sample',
    ...
    # parser.add_argument("--total_itrs", type=int, default=16e3,
    #                     help="epoch number (default: 30k)")
    parser.add_argument("--total_itrs", type=int, default=66900,
                        help="epoch number (default: 30k)")
    ...
    # parser.add_argument("--ckpt", default='D:/checkpoint/Segmentation/camvid/new_torch_deeplabv3plus/original/_1_deeplabv3plus_resnet50_camvid_sample_os16.pth', type=str,
...
def main():
    ...
    if opts.test_only:
        ...
        while True: #cur_itrs < opts.total_itrs:
            ...
            checkpoint = torch.load(f'D:/checkpoint/Segmentation/kitti/original/firstfold/_{cur_epochs}_deeplabv3plus_resnet50_kitti_sample_os16.pth', map_location=torch.device('cpu'))
            ...
            print("===>Testing using weights: ", f'D:/checkpoint/Segmentation/kitti/original/firstfold/_{cur_epochs}_deeplabv3plus_resnet50_kitti_sample_os16')
            model.cuda()
            ...
            with torch.no_grad():
                ...
                ## val loss
                    if (cur_itrs) % opts.val_interval == 0:
                        ...
                        print("---------Epoch %d, Itrs %d/%d, val_Loss=%f----------" %
                              (cur_epochs, cur_itrs, opts.total_itrs, internval_loss_plt))
                        total_val_loss.append(internval_loss_plt)
                    ...
                    if (cur_itrs) % opts.val_interval == 0:
                        ...
                        print("---------Epoch %d, Itrs %d/%d, val_Miou=%f----------" %
                              (cur_epochs, cur_itrs, opts.total_itrs, train_val_score['Mean IoU']))

                        total_val_miou.append(train_val_score['Mean IoU'])

                    if cur_itrs >= opts.total_itrs:
                        df_train_loss = pd.DataFrame(total_val_loss)
                        df_train_miou = pd.DataFrame(total_val_miou)

                        df_train_miou.to_csv('D:/plt/segmentation/kitti_original/train_miou_loss_retry_2/train_miou_2.csv', index=False)
                        df_train_loss.to_csv('D:/plt/segmentation/kitti_original/train_miou_loss_retry_2/train_loss_2.csv', index=False)
        ...
        df_train_miou.to_csv('D:/plt/segmentation/kitti_original_retry_44/train_miou.csv', index=False)
        df_train_loss.to_csv('D:/plt/segmentation/kitti_original_retry_44/train_loss.csv', index=False)
