...
from datasets import  Camvid_Edge, Kitti_sample, Camvid_proposed, Camvid_proposed_each_0, Camvid_sample, Camvid_sample_edge_4labels
...
from network.unet import UNet_3Plus
...
def get_argparser():
    ...
    parser.add_argument("--data_root", type=str, default='D:/Dataset/Camvid/camvid_sample_2/blur/psf_15_3',
    ...
    parser.add_argument("--dataset", type=str, default='camvid_sample',
                        choices=['camvid_sample', 'Edge', 'camvid', 'camvid_edge_4labels'], help='Name of dataset')
    ...
    parser.add_argument("--ckpt", default='E:/Second_paper/Checkpoint/Camvid/Blur_firstfold/main_11_unetmy_3_0,1,0_lr_0.0001_after_relu/best_deeplabv3plus_resnet50_camvid_sample_os16.pth', type=str,
...
def get_dataset(opts):
    ...
    if opts.dataset == 'camvid_edge_4labels':
        train_transform = et.ExtCompose([
            # et.ExtResize( 512 ),
            et.ExtRandomCrop(size=(opts.crop_size, opts.crop_size)),
            # et.ExtColorJitter(brightness=0.5, contrast=0.5, saturation=0.5),
        ...
        train_dst = Camvid_sample_edge_4labels(root=opts.data_root, split='train', transform=train_transform)

        val_dst = Camvid_sample_edge_4labels(root=opts.data_root, split='val', transform=val_transform)

        test_dst = Camvid_sample_edge_4labels(root=opts.data_root, split='test', transform=val_transform)
...
def validate(opts, model, loader, device, metrics, ret_samples_ids=None):
    ...
    with torch.no_grad():
        for i, (images, labels) in tqdm(enumerate(loader)):
            ...
            outputs, outputs_1, outputs_2, outputs_3 = model(images)
...
def main():
    ...
    model = UNet_3Plus(n_channels=3, n_classes=12)
    ...
    if opts.ckpt is not None and opts.test_only:
        ...
        # print(model)
        # torch.save(model, 'E:/Second_paper/Checkpoint/Camvid/EdgeNet_2_model/model.pt')
...
######### last checkpoint 받아서 이어서 학습 ##############
        ...
        while True: #cur_itrs < opts.total_itrs:
            ...
            for (images, labels) in train_loader:
                ...
                # labels = labels.to(device, dtype=torch.long)
                labels_1 =  FF.resize(labels, 30, interpolation=Image.NEAREST)
                labels_2 =  FF.resize(labels, 60, interpolation=Image.NEAREST)
                labels_3 =  FF.resize(labels, 120, interpolation=Image.NEAREST)

                labels = labels.to(device, dtype=torch.long)
                labels_1 = labels_1.to(device, dtype=torch.long)
                labels_2 = labels_2.to(device, dtype=torch.long)
                labels_3 = labels_3.to(device, dtype=torch.long)
                ...
                pred_1 = torch.max(outputs_1, 1, keepdim=True)[0]
                pred_2 = torch.max(outputs_2, 1, keepdim=True)[0]
                pred_3 = torch.max(outputs_3, 1, keepdim=True)[0]

                labels_1 = labels_1.to(device, dtype=torch.float32)
                labels_1 = torch.unsqueeze(labels_1, 1)
                loss_perceptual_1 = criterion_edge_perceptual(pred_1, labels_1)

                labels_2 = labels_2.to(device, dtype=torch.float32)
                labels_2 = torch.unsqueeze(labels_2, 1)
                loss_perceptual_2 = criterion_edge_perceptual(pred_2, labels_2)

                labels_3 = labels_3.to(device, dtype=torch.float32)
                labels_3 = torch.unsqueeze(labels_3, 1)
                loss_perceptual_3 = criterion_edge_perceptual(pred_3, labels_3)
                ...
                loss = loss + loss_perceptual + 0.5 * loss_perceptual_1 + 0.25 * loss_perceptual_2 + 0.125 * loss_perceptual_3
                ...
                if (cur_itrs) % opts.val_interval == 0:
                    ...
                    save_ckpt('E:/Second_paper/Checkpoint/Kitti/Segmentation/Blurred/Firstfold_3/main_10_2_unet3plus_1,0.5,0.25,0.05_0.0001/latest_%s_%s_os%d.pth' %
                              (opts.model, opts.dataset, opts.output_stride))
                    print("validation...")
                    model.eval()
                    val_score, ret_samples = validate(
                    ...
                    if val_score['Mean IoU'] > best_score:  # save best model
                        ...
                        #           (opts.model, opts.dataset,opts.output_stride))
                        save_ckpt('E:/Second_paper/Checkpoint/Kitti/Segmentation/Blurred/Firstfold_3/main_10_2_unet3plus_1,0.5,0.25,0.05_0.0001/best_%s_%s_os%d.pth' %
                                  (opts.model, opts.dataset, opts.output_stride))
                    ...
                    save_ckpt('E:/Second_paper/Checkpoint/Kitti/Segmentation/Blurred/Firstfold_3/main_10_2_unet3plus_1,0.5,0.25,0.05_0.0001/_%s_%s_%s_os%d.pth' %
                ...
                if cur_itrs >= opts.total_itrs:
                    ...
                    df_train_miou.to_csv('E:/Second_paper/Checkpoint/Kitti/Segmentation/Blurred/Firstfold_3/main_10_2_unet3plus_1,0.5,0.25,0.05_0.0001/train_miou_2.csv', index=False)
                    df_train_loss.to_csv('E:/Second_paper/Checkpoint/Kitti/Segmentation/Blurred/Firstfold_3/main_10_2_unet3plus_1,0.5,0.25,0.05_0.0001/train_loss_2.csv', index=False)
