...
class EuclideanCodebook(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.kmeans_init:
            self._kmeans_init(flatten_x)
        distance = torch.cdist(flatten_x, self.embedding.weight.to(device='cuda'), p=2) # (N, 모든 픽셀 수, num_embeddings)
        embed_idx = torch.argmin(distance, dim=-1) # (N, 모든 픽셀 수)
        embed_idx_onehot = F.one_hot(embed_idx, num_classes=self.num_embeddings) # (N, 모든 픽셀 수, num_embeddings)
        quantized = torch.matmul(embed_idx_onehot.float(), self.embedding.weight.to(device='cuda'))
