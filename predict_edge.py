...
from datasets import VOCSegmentation, Cityscapes, cityscapes, Camvid, Dacon_segmentation, Camvid_Edge, Camvid_sample, Camvid_Edge_laplacian
...
def get_argparser():
    ...
    parser.add_argument("--input", type=str,
                        default='D:/Dataset/Camvid/Camvid_Edge_laplacian',
                        # required=True
                        help="path to a single image or image directory")
    # parser.add_argument("--dataset", type=str, default='camvid',
    #                     choices=['voc', 'cityscapes', 'camvid'], help='Name of training set')
    parser.add_argument("--dataset", type=str, default='camvid_edge_laplacian',
                        choices=['voc', 'cityscapes, camvid', 'camvid_open', 'camvid_45', 'camvid_60',
                                 'camvid_deblur45_v2', 'dacon', 'camvid_sample', 'camvid_edge', 'camvid_edge_laplacian'], help='Name of dataset')
    # Deeplab Options
    ...
    parser.add_argument("--save_val_results_to",
                        default='E:/Second_paper/Result_Image/Edge/edge_result/Laplacian',
    ...
    parser.add_argument("--ckpt",
                        default='E:/Second_paper/Checkpoint/Camvid/EdgeNet_laplacian/best_deeplabv3plus_resnet50_Edge_laplacian_os16.pth',
                        type=str,
...
def main():
    ...
    elif opts.dataset.lower() == 'camvid_edge_laplacian':
        opts.num_classes = 2
        decode_fn = Camvid_Edge_laplacian.decode_target
    ...
    with torch.no_grad():
        ...
        for img_path in tqdm(image_files):
            ...
            img = img.to(device)
            # asd = model(img)
            # predaasd = model(img).max(1)
            # predaasd = model(img).max(1)[1]
